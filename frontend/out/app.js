...
(() => {
  ...
  function updateToday() {
    ...
    else { card.classList.add('pending'); card.classList.remove('done'); $('todayTitle').textContent = 'Conferência pendente'; $('todayText').textContent = 'Ainda não há uma aplicação registrada hoje.'; $('todayStatus').querySelector('.status-icon').textContent = '⏳'; }
  }
  ...
  function renderRecords() {
    ...
    data.forEach(r => {
      ...
      card.innerHTML = `<span class="record-icon">💉</span><div><strong>${escapeHtml(r.medicine || 'Medicamento')}</strong><small>${r.date} · ${r.time}</small><small>Dose: ${escapeHtml(r.dose)} mcg · Concentração: ${escapeHtml(r.concentration)} mg/mL</small></div><strong class="volume">${escapeHtml(r.volume)} mL</strong>`;
  ...
  function exportPdf() {
    if (!storage.flag(KEYS.pro)) { showToast('⭐ Este recurso faz parte do Seringa Pro.'); switchTab('pro'); return; }
  ...
  function activatePro() {
    ...
    if (storage.setFlag(KEYS.pro, true)) { msg.textContent = '✓ Seringa Pro desbloqueado neste dispositivo.'; msg.className = 'activation-message good'; updateProUI(); showToast('⭐ Seringa Pro ativado!', true); }
  ...
  function setupInstall() {
    ...
    $('installBanner').addEventListener('click', async () => {
      ...
      if (ios) $('iosModal').showModal(); else showToast('Use o menu do navegador e escolha “Instalar aplicativo”.');
    });
  ...
  function init() {
    ...
    renderRecords(); updateToday(); updateProUI(); setupChecklist(); setupHero(); setupEvents(); setupInstall(); recalculate();
