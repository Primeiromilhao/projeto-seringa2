/* SERINGA PWA â€” Vanilla JS, Local-First e sem dependÃªncias externas. */
(() => {
  'use strict';
  ...
  // localStorage nunca deve impedir a abertura da aplicaÃ§Ã£o.
  const storage = {
  ...
  // Converte decimal para BigInt escalado, sem usar ponto flutuante no cÃ¡lculo.
  ...
  // dose (mcg) / concentraÃ§Ã£o (mg/mL), com arredondamento final para 2 casas.
  ...
  function updateToday() {
    ...
    if (today.length) { card.classList.remove('pending'); card.classList.add('done'); $('todayTitle').textContent = 'AplicaÃ§Ã£o registrada hoje'; $('todayText').textContent = `${today.length} registro${today.length > 1 ? 's' : ''} feito${today.length > 1 ? 's' : ''} hoje. âœ“`; $('todayStatus').querySelector('.status-icon').textContent = 'âœ“'; }
    else { card.classList.add('pending'); card.classList.remove('done'); $('todayTitle').textContent = 'ConferÃªncia pendente'; $('todayText').textContent = 'Ainda nÃ£o hÃ¡ uma aplicaÃ§Ã£o registrada hoje.'; $('todayStatus').querySelector('.status-icon').textContent = 'â³'; }
  }
  ...
  function renderRecords() {
    ...
    data.forEach(r => {
      ...
      card.innerHTML = `<span class="record-icon">ðŸ’‰</span><div><strong>${escapeHtml(r.medicine || 'Medicamento')}</strong><small>${r.date} Â· ${r.time}</small><small>Dose: ${escapeHtml(r.dose)} mcg Â· ConcentraÃ§Ã£o: ${escapeHtml(r.concentration)} mg/mL</small></div><strong class="volume">${escapeHtml(r.volume)} mL</strong>`;
  ...
  function recalculate() {
    ...
    $('volume').textContent = result ? `${result.text} mL` : 'â€”'; $('math').textContent = result ? 'CÃ¡lculo exato com inteiros escalados; arredondamento somente na apresentaÃ§Ã£o.' : 'Preencha dose e concentraÃ§Ã£o para visualizar.'; updateRuler(result); return result;
  }
  ...
  function saveRecord() {
    ...
    if (!result) { showToast('Preencha a dose e a concentraÃ§Ã£o para continuar.'); $('dose').focus(); return; }
    const data = records(); const now = new Date();
    data.push({ iso: now.toISOString(), dateKey: todayKey(), date: now.toLocaleDateString('pt-PT'), time: now.toLocaleTimeString('pt-PT', {hour:'2-digit',minute:'2-digit'}), medicine: $('medicine').value.trim(), dose: $('dose').value, concentration: $('conc').value, vial: $('vial').value, volume: result.text });
    if (!storage.set(KEYS.records, data)) { showToast('NÃ£o foi possÃ­vel guardar neste dispositivo.'); return; }
    renderRecords(); updateToday(); showToast('âœ… AplicaÃ§Ã£o registrada com sucesso!', true); switchTab('records');
  }
  ...
  function audioSupport() {
    if (!('speechSynthesis' in window)) { showToast('Ãudio de apoio nÃ£o estÃ¡ disponÃ­vel neste navegador.'); return; }
    speechSynthesis.cancel(); const u = new SpeechSynthesisUtterance('Respire com calma. Confira cada etapa no seu ritmo. VocÃª pode pausar sempre que precisar.'); u.lang = 'pt-PT'; u.rate = .85; speechSynthesis.speak(u); showToast('ðŸ”Š Ãudio de apoio iniciado.');
  }
  ...
  function activatePro() {
    ...
    const valid = /^SERINGA-PRO-[A-Z0-9]{6,}$/.test(key);
    if (!valid) { msg.textContent = 'Chave invÃ¡lida. Confira a chave e tente novamente.'; msg.className = 'activation-message bad'; return; }
    if (storage.setFlag(KEYS.pro, true)) { msg.textContent = 'âœ“ Seringa Pro desbloqueado neste dispositivo.'; msg.className = 'activation-message good'; updateProUI(); showToast('â­ Seringa Pro ativado!', true); }
    else { msg.textContent = 'NÃ£o foi possÃ­vel guardar a ativaÃ§Ã£o localmente.'; msg.className = 'activation-message bad'; }
  }
  ...
  function setupInstall() {
    ...
    $('installBanner').addEventListener('click', async () => {
      ...
      if (ios) $('iosModal').showModal(); else showToast('Use o menu do navegador e escolha â€œInstalar aplicativoâ€.');
    });
    window.addEventListener('appinstalled', () => { deferredInstall = null; $('installBanner').hidden = true; showToast('ðŸ“² SERINGA instalado com sucesso!', true); });
  }

  function setupChecklist() {
    const boxes = [...document.querySelectorAll('#checklist input')];
    boxes.forEach(b => b.addEventListener('change', () => { const n = boxes.filter(x => x.checked).length; $('progress').textContent = `${n}/4`; const f = $('checkFeedback'); f.classList.toggle('good', n > 0); f.textContent = n === 4 ? 'âœ“ Tudo conferido. VocÃª concluiu esta etapa com calma.' : n ? `${n} de 4 etapas concluÃ­das. Muito bem, siga no seu ritmo.` : 'Vamos com calma. Marque cada etapa quando estiver pronta.'; }));
  }
  ...
  function setupEvents() {
    ...
    $('clear').addEventListener('click', () => $('confirmModal').showModal()); $('confirmClear').addEventListener('click', () => { storage.remove(KEYS.records); renderRecords(); updateToday(); $('confirmModal').close(); showToast('HistÃ³rico limpo neste dispositivo.'); });
